...
# Must have in the same folder the "evaluation" folder of VGAVAE, the parabart.py script and its model folder,
# the transformers folder. root must point to a folder with the ParaNMT triplets.
...
def get_parabart_eval():
    ...
    return np.mean(syn_res_sims)*100, np.mean(sem_res_sims)*100, np.mean(para_sims)*100
...
def get_tma():
    ...
    return syn2res_tma2, syn2res_tma3, sem2res_tma2, sem2res_tma3, para2res_tma2, para2res_tma3
...
def get_all_M_STD():
    ...
    return syn_M, syn_STD, sem_M, sem_STD, para_M, para_STD
...
for file in res_fs:
    ...
    syn_PB, sem_PB, para_PB = get_parabart_eval()
    t1 = time()
    print("############### Parabart Evaluation took {} seconds ##############".format(t1-t0), flush=True)

    print("############### Getting TMA Evaluation ##############", flush=True)
    syn_tma2, syn_tma3, sem_tma2, sem_tma3, para_tma2, para_tma3 = get_tma()
    t2 = time()
    ...
    syn_M, syn_STD, sem_M, sem_STD, para_M, para_STD = get_all_M_STD()
    t3 = time()
    ...
    numbers.append([sem_M, sem_PB, sem_STD, sem_tma2, sem_tma3, syn_M, syn_PB, syn_STD, syn_tma2, syn_tma3, para_M,
...
print("{:^5s}|||{:^45s}|||{:^45s}|||{:^45s}".format("File", "sem_src", "syn_src", "para"))
