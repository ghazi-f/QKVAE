...
class BARTNLI:
    def __init__(self, max_len, batch_size, max_epochs, device, unsup_proportion=1., sup_proportion=1., dev_index=1,
        ...
        np.random.seed(42)
        # Loading Data
        folder = os.path.join(".data", "nli_gen", "nli_gen")
        ...
        self.dataset = load_dataset('csv', data_files={'train': train_path, 'valid': valid_path, 'test': test_path},
                                    delimiter='\t', column_names=['text'], keep_in_memory=True)
...
class BARTYelp:
    ...
    def __init__(self, max_len, batch_size, max_epochs, device, unsup_proportion=1., sup_proportion=1., dev_index=1,
        ...
        np.random.seed(42)
        # Loading Data
        folder = os.path.join(".data", "binary_yelp")
        train_path, valid_path, test_path = os.path.join(folder, 'yelp.train.tsv'), os.path.join(folder, 'yelp.dev.tsv'), \
                                os.path.join(folder, 'yelp.test.tsv')
        self.dataset = load_dataset('csv', data_files={'train': train_path, 'valid': valid_path, 'test': test_path},
                                    delimiter='\t', column_names=['text', 'label'], keep_in_memory=True)
    ...
    def shuffle_split(self, split):
        ...
        np.random.shuffle(rrange)
        self.dataset[split]['text'] = np.array(self.dataset[split]['text'])[rrange].tolist()
        self.dataset[split]['label'] = np.array(self.dataset[split]['label'])[rrange].tolist()
...
class BARTBookCorpus:
    def __init__(self, max_len, batch_size, max_epochs, device, unsup_proportion=1., sup_proportion=1., dev_index=1,
        ...
        # Setting up iterators
        self.shuffle_split('train'), self.shuffle_split('valid'), self.shuffle_split('test')
        self.train_iter, self.enc_train_iter = MyNextIter(self, self.dataset['train']), \
                                               MyNextIter(self, self.dataset['train'])
        self.val_iter, self.test_iter = MyNextIter(self, self.dataset['valid'], divide_bs=self.divide_bs), \
                                        MyNextIter(self, self.dataset['test'], divide_bs=self.divide_bs)
    ...
    def reinit_iterator(self, split):
        if split == 'train':
            ...
            if self.n_epochs < self.max_epochs:
                self.shuffle_split('train')
                self.train_iter = MyNextIter(self, self.dataset['train'])
            else:
                print("Reached n_epochs={} and finished training !".format(self.n_epochs))
                self.train_iter = None
            ...
            self.shuffle_split('valid')
            self.val_iter = MyNextIter(self, self.dataset['valid'], divide_bs=self.divide_bs)
        elif split == 'test':
            self.shuffle_split('test')
...
class BARTWiki:
    ...
    def __init__(self, max_len, batch_size, max_epochs, device, unsup_proportion=1., sup_proportion=1., dev_index=1,
        ...
        np.random.seed(42)
        # Loading Data
        folder = os.path.join(".data", "wiki21")
        train_path, valid_path, test_path = os.path.join(folder, 'train.txt'), os.path.join(folder, 'dev.txt'), \
                                            os.path.join(folder, 'test.txt')
        self.dataset = load_dataset('csv', data_files={'train': train_path, 'valid': valid_path, 'test': test_path},
                                    delimiter='\t', column_names=['text', 'para'], keep_in_memory=True)
