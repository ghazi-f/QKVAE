...
from components.links import CoattentiveTransformerLink, ConditionalCoattentiveTransformerLink, \
    ...
    BARTHEZ_LINK, BART_LINK, AutoModel, LOCAL_ONLY
...
class DisentanglementTransformerVAE(nn.Module, metaclass=abc.ABCMeta):
    def __init__(self, vocab_index, tag_index, h_params, autoload=True, wvs=None, dataset=None):
        ...
        if self.uses_bart:
            self.word_embeddings = AutoModel.from_pretrained(BARTHEZ_LINK, local_files_only=LOCAL_ONLY).shared if h_params.fr else \
                BartModel.from_pretrained(BART_LINK, local_files_only=LOCAL_ONLY).shared
        else:
...
class StructuredDisentanglementVAE(nn.Module, metaclass=abc.ABCMeta):
    def __init__(self, vocab_index, tag_index, h_params, autoload=True, wvs=None, dataset=None):
        ...
        super(StructuredDisentanglementVAE, self).__init__()
        ...
        # Semantic Graph initial state
        print(h_params.n_aux_mem, h_params.z_size)
        self.init_g = nn.Embedding(h_params.n_aux_mem, h_params.z_size).weight
        ...
        self.aux_bn = BayesNet(vertices['aux'])
    ...
    def opt_step(self, samples):
        ...
        if self.h_params.lv_kl_coeff > 0:
            deactivate_all_attention_outputs()
        zprev_size = int(self.h_params.z_size*(self.h_params.n_latents[0]-1)/self.h_params.n_latents[0])
        gen_inputs = {**{k.name: v for k, v in self.infer_bn.variables_hat.items()},
                      **{'x': samples['x'], 'x_prev': samples['x_prev']},
                      'z_prev1':self.infer_bn.variables_hat[self.infer_bn.name_to_v['z1']][..., :zprev_size]}
        if self.iw:
            gen_inputs = self._harmonize_input_shapes(gen_inputs, self.h_params.training_iw_samples)
        if self.step < self.h_params.anneal_kl[0] and self.h_params.anneal_kl_type == "linear":
            self.gen_last_states = self.gen_bn(gen_inputs, target=self.generated_v,
    ...
    def forward(self, samples, eval=False, prev_states=None, force_iw=None, sem_loss=True):
        # Just propagating values through the bayesian networks to get summaries
        if prev_states:
            infer_prev, gen_prev = prev_states
        else:
    ...
    def structured_prior(self, orig_zp_sample=None, shape=None):
        shape = shape or (1,)
        zp_gen = self.gen_bn.name_to_v['zp']
    ...
    def data_specific_metrics(self):
        ...
        with torch.no_grad():
            ...
            gen_inputs = {**{lv.name: lv.infer(lv.post_params) for lv in self.infer_bn.variables if lv.name.startswith('z')},
                          **{'x': self.gen_bn.variables_star[self.gen_bn.name_to_v['x_prev']],
                             'x_prev': self.gen_bn.variables_star[self.gen_bn.name_to_v['x_prev']], 'z_prev1':None}}
            self.gen_bn(gen_inputs, target=self.generated_v, complete=True)
            summary_triplets.append(
                ('text', '/reconstructions', self.decode_to_text(self.generated_v.post_params['logits']))
            )
            n_samples = sum(self.h_params.n_latents) + 2
            repeats = 2
            go_symbol = torch.ones([n_samples*repeats + 2]).long() * self.index[self.generated_v].stoi[self.go_symbol]
            go_symbol = go_symbol.to(self.h_params.device).unsqueeze(-1)
            x_prev = go_symbol
            gen_len = self.h_params.max_len

            orig_zp_sample1, orig_z_sample1, orig_zst_sample_1 = self.structured_prior()
            orig_zp_sample2, orig_z_sample2, orig_zst_sample_2 = self.structured_prior()

            z_samples_1 = orig_z_sample1.repeat(n_samples+1, 1)
            z_samples_2 = orig_z_sample2.repeat(n_samples+1, 1)

            zst_samples_1 = torch.cat([orig_zst_sample_1.repeat(n_samples, 1), orig_zst_sample_2.repeat(1, 1)])
            zst_samples_2 = torch.cat([orig_zst_sample_2.repeat(n_samples, 1), orig_zst_sample_1.repeat(1, 1)])
            zp_samples_1 = torch.cat([orig_zp_sample1.repeat(n_samples-1, 1), orig_zp_sample2.repeat(1, 1),
                                       orig_zp_sample1.repeat(1, 1)])
            zp_samples_2 = torch.cat([orig_zp_sample2.repeat(n_samples-1, 1), orig_zp_sample1.repeat(1, 1),
                                       orig_zp_sample2.repeat(1, 1)])
            # Swapping latent variable values
            for i in range(1, self.h_params.n_latents[0] + 1):
                start, end = int((i - 1) * self.h_params.z_size / max(self.h_params.n_latents)), \
                             int(i * self.h_params.z_size / max(self.h_params.n_latents))
                z_samples_1[i, ..., start:end] = orig_z_sample2[0, ..., start:end]
                z_samples_2[i, ..., start:end] = orig_z_sample1[0, ..., start:end]
    ...
    def decode_to_text(self, x_hat_params, gen=False):
        ...
        if not gen:
            ...
            first_sample, second_sample = samples[:int(len(samples)/2)], samples[int(len(samples) / 2):]
            samples = ['**First Sample**\n'] + \
                      [('orig' if i == 0 else 'zs' if i == len(first_sample)-1 else 'zp' if i == len(first_sample)-2
                      else str(i) if sample == first_sample[0] else '**'+str(i)+'**') + ': ' +
                       sample for i, sample in enumerate(first_sample)] + \
                      ['**Second Sample**\n'] + \
                      [('orig' if i == 0 else 'zs' if i == len(second_sample)-1 else 'zp' if i == len(second_sample)-2
                      else str(i) if sample == second_sample[0] else '**'+str(i)+'**') + ': ' +
                       sample for i, sample in enumerate(second_sample)]
            text = ' |||| '.join(samples).replace('<pad>', '_').replace('_unk', '<?>').replace(' ğ', '')
    ...
    def eval(self):
        for v in self.infer_bn.variables | self.gen_bn.variables:
            if v.name != self.generated_v.name and (isinstance(v, MultiCategorical) or isinstance(v, Categorical)):
                v.switch_to_non_relaxed()
        super(StructuredDisentanglementVAE, self).eval()

    def train(self, mode=True):
        for v in self.infer_bn.variables | self.gen_bn.variables:
            if v.name != self.generated_v.name and (isinstance(v, MultiCategorical) or isinstance(v, Categorical)):
                v.switch_to_relaxed()
        super(StructuredDisentanglementVAE, self).train(mode=mode)
    ...
    def _get_alternative_sentences(self, prev_latent_vals, params, var_z_ids, n_samples, gen_len, complete=None):
        ...
        orig_zst = prev_latent_vals['zs'].repeat(n_samples, 1)
        orig_zp = prev_latent_vals['zp'].repeat(n_samples, 1)

        zp_sample, z_sample, zst_sample = self.structured_prior(shape=(n_samples*n_orig_sentences, ))

        for id in var_z_ids:
            # id == sum(h_params.n_latents) means its zst
            if id == sum(h_params.n_latents)+1:
                orig_zp = zp_sample
            elif id == sum(h_params.n_latents):
                orig_zst = zst_sample
            else:
                assert id < sum(h_params.n_latents)
                z_number = sum([id > sum(h_params.n_latents[:i + 1]) for i in range(len(h_params.n_latents))])
                ...
                start, end = int(h_params.z_size / max(h_params.n_latents) * z_index), int(
                    h_params.z_size / max(h_params.n_latents) * (z_index + 1))
                orig_z[:, start:end] = z_sample[:, start:end]
        z_input = {'z1': orig_z.unsqueeze(1), 'zs': orig_zst.unsqueeze(1), 'zp': orig_zp.unsqueeze(1)}

        x_prev = self.generate_from_z2(z_input, x_prev, beam_size=self.beam_size, gen_len=gen_len)
        text = self.decode_to_text2(x_prev, self.h_params.vocab_size, self.index[self.generated_v])
    ...
    def get_sentences(self, n_samples, gen_len=16, sample_w=False, vary_z=True, complete=None, contains=None,
        ...
        while n_samples > 0:
            ...
            zp_sample, z_sample, zst_sample = self.structured_prior(shape=(n_samples, ))
            z_params = {'z1': z_gen.post_params, 'zs': zst_gen.post_params, 'zp': zp_gen.post_params}

            z_input = {'z1': z_sample.unsqueeze(1), 'zs': zst_sample.unsqueeze(1), 'zp': zp_sample.unsqueeze(1)}
            x_prev = self.generate_from_z2(z_input, x_prev, beam_size=beam_size or self.beam_size, gen_len=gen_len)

            text = self.decode_to_text2(x_prev, self.h_params.vocab_size, self.index[self.generated_v])
            if contains is None:
                sample_dic = {'z1': z_sample, 'zs':zst_sample, 'zp':zp_sample}
                return text, sample_dic, z_params
            else:
                raise NotImplementedError("key word based sampling still not implemented for qkv graph")
    ...
    def get_att_and_rel_idx(self, text_in):
        ...
        activate_all_attention_outputs()
        self.infer_bn({'x': text_in}, target=[self.infer_bn.name_to_v['z1'], self.infer_bn.name_to_v['zp']])
        att_vals = self.get_enc_att_vals().cpu().detach().numpy()
        att_vals = att_vals.mean(-2)
        deactivate_all_attention_outputs()
        att_maxes = att_vals.argmax(-1).tolist()
    ...
    def get_enc_att_vals(self):
        max_len = self.h_params.max_len
        trans_mod = self.infer_bn.approximator[self.infer_bn.name_to_v['z1']]
        trans_mod_p = self.infer_bn.approximator[self.infer_bn.name_to_v['zp']]
        att_weights = []
        for lv in range(sum(self.h_params.n_latents)):
            var_att_weights = []
            for layer_att_vals in trans_mod.att_vals:
                att_out = layer_att_vals[:, lv,:max_len]
                var_att_weights.append(att_out)
                # var_att_weights.append(att_out.cpu().detach().numpy())
            # att_weights.append(var_att_weights)
            att_weights.append(torch.stack(var_att_weights))
        ...
        var_att_weights = []
        for layer_att_vals in trans_mod_p.att_vals:
            att_out = layer_att_vals[:, 0, :max_len]
            var_att_weights.append(att_out)
        att_weights.append(torch.stack(var_att_weights))
        # att_vals shape:[sent, lv, layer, tok]
        att_vals = torch.stack(att_weights).transpose(2, 1).transpose(1, 0)
    ...
    def get_sem_loss(self, samples):
        # Get latent variables for sentences and next sentences and negative samples
        zp_var, zc_var, g_var = self.infer_bn.name_to_v['zp'], self.infer_bn.name_to_v['z1'], \
                                self.aux_bn.name_to_v['g']
        infer_inputs = {'x': samples['x'],  'x_prev': samples['x_prev']}
        self.infer_bn(infer_inputs, complete=True)
        zp, zc = zp_var.post_samples, zc_var.post_samples
        infer_inputs = {'x': samples['x+'],  'x_prev': samples['x_prev+']}
        self.infer_bn(infer_inputs, complete=True)
        ...
        aux_inputs = {'zp': zp, 'z1': zc, 'g_prev': init_g}
        self.aux_bn(aux_inputs, complete=True)
        g = g_var.post_samples
        g_params = g_var.post_params

        # Get updated semantic graph distributions for updates coming from positive and negative samples
        aux_inputs = {'zp': zp_plus, 'z1': zc_plus, 'g_prev': g}
        self.aux_bn(aux_inputs)
        g_params_plus = g_var.post_params
        aux_inputs = {'zp': zp_minus, 'z1': zc_minus, 'g_prev': g}
        self.aux_bn(aux_inputs)
        g_params_minus = g_var.post_params

        self.aux_bn.clear_values()
    ...
    def get_encoder_disentanglement_score(self, data_iter):
        ...
        lv_scores = []
        for lv in range(sum(self.h_params.n_latents)+1):
            found = {'subj': [], 'verb': [], 'dobj': [], 'pobj': []}
            for att, rel_pos in zip(att_maxes, rel_idx):
                for k in found.keys():
                    if len(rel_pos[k]):
        ...
        for k, v in enc_att_scores.items():
            ...
            enc_disent_vars[k], enc_disent_score[k], enc_max_score[k] = \
                sort_idx[-1], v[sort_idx[-1]] - v[sort_idx[-2]], v[sort_idx[-1]]
        idx_names = ['z{}'.format(i+1) for i in range(sum(self.h_params.n_latents))]+['zp']
    ...
    def _get_stat_data_frame2(self, n_samples=2000, n_alterations=1, batch_size=50):
        ...
        for _ in tqdm(range(int(n_samples / batch_size)), desc="Generating original sentences"):
            ...
            orig_temps.extend(truncated_template(text_i))
        for i in range(int(n_samples / batch_size)):
            for j in tqdm(range(sum(self.h_params.n_latents)+2), desc="Processing sample {}".format(str(i))):
                # Altering the sentences
                alt_text, _ = self._get_alternative_sentences(
                    prev_latent_vals={k: v[i * batch_size:(i + 1) * batch_size]
                                      for k, v in samples.items()},
                ...
                for k in range(n_alterations * batch_size):
                    ...
                    try:
                        ...
                        continue
                    stats.append([orig_text, alt_text[k], (str(j) if j < sum(self.h_params.n_latents) else
                                                           'zs' if j ==sum(self.h_params.n_latents) else 'zp'),
                                  *(int(diff[0]) for diff in r_diffs),*(int(diff[1]) for diff in r_diffs),
                                  *(int(diff[2]) for diff in r_diffs),same_struct, syn_temp_diff, lex_temp_diff])
        ...
        # renormalizing
        ex_array = np.array(var_wise_scores_ex)+1e-10
        ex_array = np.concatenate([ex_array, np.ones((sum(self.h_params.n_latents)+2, 2))], axis=1)
        var_wise_scores = var_wise_scores/ex_array
    ...
    def get_swap_tma(self, n_samples=2000, batch_size=50, beam_size=None):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(int(n_samples / (2 * batch_size))),
                          desc="Getting Model Swap TMA"):
                z_input = {'zs': source_lvs['zs'][i * batch_size:(i + 1) * batch_size].unsqueeze(1),
                           'z1': target_lvs['z1'][i * batch_size:(i + 1) * batch_size].unsqueeze(1),
                           'zp': target_lvs['zp'][i * batch_size:(i + 1) * batch_size].unsqueeze(1)}
                inv_z_input = {'zs': target_lvs['zs'][i * batch_size:(i + 1) * batch_size].unsqueeze(1),
                               'z1': source_lvs['z1'][i * batch_size:(i + 1) * batch_size].unsqueeze(1),
                               'zp': source_lvs['zp'][i * batch_size:(i + 1) * batch_size].unsqueeze(1)}
                x_prev = go_symbol.repeat((batch_size, 1))
                x_prev = self.generate_from_z2(z_input, x_prev, mask_unk=False, beam_size=beam_size)
                result_sents.extend(self.decode_to_text2(x_prev, self.h_params.vocab_size,
                                                         self.index[self.generated_v]))
    ...
    def collect_stats(self, data_iter):
        kl, kl_var, rec, mi, nsamples = 0, 0, 0, 0, 0
        varwise_mi = {'z1': 0, 'zs':0, 'zp':0}
        infer_prev, gen_prev = None, None
        loss_obj = self.losses[0]
        zs = {'z1': (self.infer_bn.name_to_v['z1'], self.gen_bn.name_to_v['z1']),
              'zs': (self.infer_bn.name_to_v['zs'], self.gen_bn.name_to_v['zs']),
              'zp': (self.infer_bn.name_to_v['zp'], self.gen_bn.name_to_v['zp'])}
        with torch.no_grad():
            for i, batch in enumerate(tqdm(data_iter, desc="Getting Model Stats")):
                if batch.text.shape[1] < 2: continue
                infer_prev, gen_prev = self({'x': batch.text[..., 1:],'x_prev': batch.text[..., :-1]},
                                            prev_states=(infer_prev, gen_prev), sem_loss=False)
                if not self.h_params.contiguous_lm:
                    infer_prev, gen_prev = None, None
                nsamples += batch.text.shape[0]
                kl += sum([v for k, v in loss_obj.KL_dict.items() if not k.startswith('/Var')]) * batch.text.shape[0]
    ...
    def generate_from_z(self, z_input, x_prev, gen_len=None, only_z_sampling=True, temp=1.0, mask_unk=True):
        ...
        for i in range(gen_len or self.h_params.max_len):
            self.gen_bn({'x_prev': x_prev, **{k: v.expand(v.shape[0], i + 1, v.shape[-1])
                                              for k, v in z_input.items()}, 'z_prev1': None}, target=self.generated_v)
            unk_mask_i = unk_mask.expand(unk_mask.shape[0], i + 1, unk_mask.shape[-1])
            if only_z_sampling:
                samples_i = self.generated_v.post_params['logits']
            else:
    ...
    def generate_from_z2(self, z_input, x_prev, gen_len=None, only_z_sampling=True, temp=1.0, mask_unk=True,
        ...
        for i in range(gen_len or self.h_params.max_len):
            if beam_size == 1:
                ...
                z_i = {k: v.expand(beam_size, v.shape[1], i + 1, *v.shape[3:]) for k, v in z_input.items()}
            self.gen_bn({'x_prev': x_prev, **z_i, 'z_prev1': None}, target=self.gen_bn.name_to_v['x'])
            unk_mask_i = unk_mask.expand(*unk_mask.shape[:-2], i + 1, unk_mask.shape[-1])
            if only_z_sampling:
                samples_i = self.generated_v.post_params['logits']
            else:
    ...
    def embed_sents(self, sents, sample=False):
        with torch.no_grad():
            zs_infer, z_infer, zp_infer, x_gen = self.infer_bn.name_to_v['zs'], self.infer_bn.name_to_v['z1'],\
                                                 self.infer_bn.name_to_v['zp'], self.gen_bn.name_to_v['x']
            ...
            self.infer_bn({'x': inputs}, complete=sample)
            if sample:
                orig_zs, orig_z, orig_zp = zs_infer.post_samples[..., 0, :], z_infer.post_samples[..., 0, :], \
                                           zp_infer.post_samples[..., 0, :]
            else:
                orig_zs, orig_z, orig_zp = zs_infer.rep(zs_infer.infer(zs_infer.post_params))[..., 0, :], \
                                           z_infer.rep(z_infer.infer(z_infer.post_params))[..., 0, :], \
                                           zp_infer.rep(zp_infer.infer(zp_infer.post_params))[..., 0, :]

            return orig_zs, orig_z, orig_zp
