...
def get_residual_transformer_graph_postag(h_params, word_embeddings, pos_embeddings):
    ...
    # Inference
    yval_inf = YvalInfer(h_params, pos_embeddings)
    x_inf, yemb_inf, z_inf = XInfer(h_params, word_embeddings, has_rep=False), YEmbInfer(h_params), ZInfer(h_params)
    x_x_prev_yemb_to_z = TransformerLink(xin_size, h_params.encoder_h, zout_size, h_params.encoder_l, Gaussian.parameters,
                                         highway=h_params.highway, dropout=h_params.dropout,
                                         residual={'conditions': ['x_prev', 'yemb'], 'link': xprev_yemb_to_z},
                                         bidirectional=True)
    x_xprev_to_yemb = TransformerLink(xin_size, h_params.pos_h, yembout_size, h_params.pos_l, Gaussian.parameters,
                                      highway=h_params.highway, dropout=h_params.dropout,
                                      residual={'conditions': ['x_prev'], 'link': xprev_to_yemb},
                                      bidirectional=True)
    yemb_to_yval = MLPLink(yembin_size, h_params.pos_h, yvalout_size, h_params.pos_l, Categorical.parameters,
    ...
    return {'infer': nn.ModuleList([nn.ModuleList([x_inf, x_xprev_to_yemb, yemb_inf]),
                                    nn.ModuleList([x_prev_gen, x_xprev_to_yemb, yemb_inf]),
                                    nn.ModuleList([x_inf, x_x_prev_yemb_to_z, z_inf]),
