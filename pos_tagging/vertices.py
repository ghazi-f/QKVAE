...
def get_graph_postag(h_params, word_embeddings, pos_embeddings):
    ...
    # Inference
    x_inf, y_inf, z_inf = XInfer(h_params, word_embeddings), YInfer(h_params, pos_embeddings), ZInfer(h_params)
    x_y_to_z = GRULink(xin_size+yin_size, h_params.encoder_h, zout_size, h_params.encoder_l, Gaussian.parameters,
                     highway=h_params.highway)
    x_to_y = GRULink(xin_size, h_params.encoder_h, yout_size, h_params.encoder_l, Categorical.parameters,
                     embedding=pos_embeddings, highway=h_params.highway)
    ...
    xprev_to_z = GRULink(xin_size, h_params.decoder_h, zout_size, h_params.decoder_l, Gaussian.parameters,
                         highway=h_params.highway)
    xprev_z_to_y = GRULink(xin_size+zin_size, h_params.decoder_h, yout_size, h_params.decoder_l, Categorical.parameters,
                         embedding=pos_embeddings, highway=h_params.highway)
    ...
    return {'infer': nn.ModuleList([nn.ModuleList([x_inf, x_y_to_z, z_inf]),
                                    nn.ModuleList([x_inf, x_to_y, y_inf]),
                                    nn.ModuleList([y_inf, x_y_to_z, z_inf])
                                    ]),
