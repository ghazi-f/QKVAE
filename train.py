...
h_params = HParams(len(data.vocab.itos), len(data.tags.itos), MAX_LEN, BATCH_SIZE, N_EPOCHS,
                   ...
                   vocab_ignore_index=data.vocab.stoi['<pad>'], decoder_h=64, decoder_l=1, encoder_h=64, encoder_l=1,
                   test_name='StructuredTestbigbatch', grad_accumulation_steps=4, optimizer_kwargs={'lr': 1e-4},
                   is_weighted=['x', 'y'])
